...
def create_feature_graphic():
    ...
    try:
        ...
        print("⚠️  Fuente Arial no encontrada, usando fuente por defecto")
        font_logo = ImageFont.load_default()
        font_tagline = ImageFont.load_default()
        font_badge = ImageFont.load_default()
        font_badge_small = ImageFont.load_default()

    print("✍️  Añadiendo texto principal...")
    ...
    print("🏷️  Creando badge de idiomas...")
    ...
    # Texto "Líder del mercado"
    sub_text = "Líder del mercado"
    ...
    print("🎵 Añadiendo elementos visuales...")
    ...
    for i, height in enumerate(wave_heights):
        ...
        y = wave_y - height // 2
        # Gradiente púrpura para las ondas
        draw.rectangle([x, y, x + wave_width, y + height], fill='#8B5CF6')

    # Flecha de transición
    arrow_x = wave_x + 120
    arrow_y = wave_y - 10
    draw.text((arrow_x, arrow_y), "→", font=font_logo, fill='#FFFFFF')
    ...
    for i, (emoji, progress) in enumerate(habits):
        y = progress_y + i * progress_gap

        # Emoji (si la fuente lo soporta, sino usar texto)
        try:
            draw.text((progress_x - 25, y - 5), emoji, font=font_tagline, fill='#FFFFFF')
        except:
            draw.text((progress_x - 25, y - 5), "•", font=font_tagline, fill='#FFFFFF')
    ...
    # Iconos de hábitos adicionales (lado derecho)
    habits_x = WIDTH - 120
    habits_y = HEIGHT // 2 - 50

    for i in range(5):
        y = habits_y + i * 20
        # Icono de hábito (círculo pequeño)
    ...
    if os.path.exists(output_path):
        ...
        if file_size_kb > 1024:
            print("⚠️  Archivo > 1MB. Considerar optimizar para Play Store")
